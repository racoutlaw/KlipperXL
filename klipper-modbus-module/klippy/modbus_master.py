...
class ModbusMaster:
    ...
    def _connect(self):
        self.read_cmd = self.mcu.lookup_command(
            "modbus_read addr=%c start=%hu count=%c")
        self.write_cmd = self.mcu.lookup_command(
            "modbus_write addr=%c reg=%hu value=%hu")
        self.coil_cmd = self.mcu.lookup_command(
            "modbus_coil addr=%c coil=%hu value=%c")
        
        self.mcu.register_response(self._handle_result, "modbus_result")
        self.mcu.register_response(self._handle_write, "modbus_write_result")
        self.mcu.register_response(self._handle_coil, "modbus_coil_result")
